...
const EnvironmentChart = () => {
  ...
  const TMIN = 18, TMAX = 28;
  const HMIN = 55, HMAX = 80;
  ...
  return (
    <div className="rounded-xl border h-full flex flex-col p-4" style={{ background: "#090f0b", borderColor: "#1a3020" }}>
      <div className="flex items-center justify-between mb-3 shrink-0">
        <div>
          <h3 className="text-sm font-semibold" style={{ color: "#e2e8f0", fontFamily: "Syne, sans-serif" }}>
            24시간 환경 추이
          </h3>
          <p className="text-[11px] mt-0.5" style={{ color: "#374151" }}>오늘 온도 및 습도 변화</p>
        </div>
      ...
      <div className="flex-1 min-h-0">
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" style={{ display: "block" }}>
          ...
          {yTicks.map((v) => {
            const y = P.t + iH - ((v - TMIN) / (TMAX - TMIN)) * iH;
            return (
              <text key={v} x={P.l - 6} y={y + 3.5} textAnchor="end" fontSize="9" fill="#2d4a35" fontFamily="JetBrains Mono, monospace">
                {v}°
              </text>
            );
          })}
          ...
          {xTicks.map((h) => {
            const x = P.l + (h / 23) * iW;
            return (
              <text key={h} x={x} y={H - 8} textAnchor="middle" fontSize="9" fill="#2d4a35" fontFamily="JetBrains Mono, monospace">
