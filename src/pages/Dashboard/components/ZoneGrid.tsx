...
const ZoneGrid = () => {
  ...
  return (
    <div className="rounded-xl border flex flex-col p-4 h-full" style={{ background: "#090f0b", borderColor: "#1a3020" }}>
      <div className="mb-3 shrink-0">
        <h3 className="text-sm font-semibold" style={{ color: "#e2e8f0", fontFamily: "Syne, sans-serif" }}>
          재배 구역 현황
        </h3>
        <p className="text-[11px] mt-0.5" style={{ color: "#374151" }}>6개 구역 · 5개 운영 중</p>
      </div>
        ...
        {ZONES.map((z) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-semibold" style={{ color: "#94a3b8" }}>{z.name}</span>
              ...
              <>
                ...
                <div>
                  ...
                  <div className="h-[2px] rounded-full" style={{ background: "#111c14" }}>
                    <div className="h-full rounded-full" style={{ width: `${z.health}%`, background: statusColor[z.status] }} />
                  </div>
                </div>
              </>
            ) : (
              <span className="text-[10px] font-mono" style={{ color: "#2d4a35" }}>비활성</span>
            )}
          </div>
        ))}
      </div>
